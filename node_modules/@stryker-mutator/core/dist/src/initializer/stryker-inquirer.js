...
class StrykerInquirer {
    ...
    async promptTestRunners(options) {
        ...
        if (options.length) {
            const choices = options.map((_) => _.name);
            choices.push(new inquirer_1.default.Separator());
            choices.push(command_test_runner_1.CommandTestRunner.runnerName);
            const answers = await inquirer_1.default.prompt({
                choices,
            ...
            return (_a = options.filter((_) => _.name === answers.testRunner)[0]) !== null && _a !== void 0 ? _a : { name: command_test_runner_1.CommandTestRunner.runnerName, pkg: null };
        }
        else {
            return { name: command_test_runner_1.CommandTestRunner.runnerName, pkg: null };
        }
    }
